...
printf_last:
	...
	j	printf_loop
printf_escapechar:	    # will handle '\\', similar set up to printf_str
	addi    $s0, $s0, 1 # moves to the next character
	lbu     $a0, 0($s0) 
	li      $v0, 11	    # should print character
	syscall		    # prints the char by ASCII value 
	j	printf_last # jumps to printf_last
printf_ret:
	#restore RTE
	lw	$fp, 24($sp)
	lw	$ra, 20($sp)
